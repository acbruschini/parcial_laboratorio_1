 ...
 *
 */
int cli_imprimirArrayClientesConAvisos(Cliente* array,int limite,Publicacion* arrayPublicacion,int limitePublicacion)
{
	int respuesta = -1;
	...
	if(array != NULL && limite > 0)
	{
		...
		for(i=0;i<limite;i++)
		{
			if (!cli_imprimir(&array[i]))
			{
				...
				{
					if(arrayPublicacion[j].idCliente == array[i].id && arrayPublicacion[j].isEmpty == 0)
					{
						pub_imprimir(&arrayPublicacion[j]);
					}
				}
				printf("\n ---------------------------- \n");
			}
